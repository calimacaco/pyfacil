...
class convert_img():
	def __init__(self,nivelLog):
		self.imagen=None
		self.log=ObLog('ConvertLogo',nivelLog)
		self.log.setNivel(nivelLog)
	...
	def procesar(self):
		...
		self.log.logger.info ('Entrada: %s' % (self.imagen))
		self.log.logger.info ('Entrada: %s' % (self.archsalida))
		...
		try:
			...
			arcsalida.write('%s*t600R%s*r0F%s*r%is%iT' % ( esc,esc,esc,ancho,alto))	#resolucion, presentacion, ancho, alto
	...
	def escalar (self, porcentaje):
		self.log.logger.info ('escalar [%s]' % (porcentaje))
	...
	def modify_img (self,opacity = 0.6):
		self.log.logger.info ('modify_img = [%s]'%(opacity))
