...
import leerregistro
import msvcrt
from  borrarpcl import objNopcl
...
##Guarda una copia del spool, en una ruta.
		...
		elif (datos[2:5].upper()=="NPC"):
			nopcl=True

		elif (datos[2:5].upper()=="REB"):
			revicion=True

		elif (datos[2:5].upper()=="STO"):
			salidastd=True

				
	return valor,impresora,ambiente,proceso,log,programa,cortaspool,nopcl,revicion,salidastd



########################## Incio del programa ###########################
if __name__ == "__main__":
	(estado,impresora,ambiente,proceso,log,programa,cortaspool,nopcl,revicion,salidastd)=BuscarArgumentos()
	if estado<6:
		sys.exit (2)
	ruta=""
	...
	if nopcl:
		...
		for linea in sys.stdin.readlines():
			linea=objpcl.procesarlinea(linea)
	...
	ejecutar= ruta + "\\"+programa+" --JOB=" + nombre + " --IMP=" + impresora + " --MOD=" + ambiente + " --PRO=" + proceso
	if salidastd==True:
		ejecutar+=" --StdOut "
